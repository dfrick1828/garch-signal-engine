...
def classify_regime(row):
    vo, vc = row.get("VIXOpenQuote", np.nan), row.get("VIXCloseQuote", np.nan)
    so, sc = row.get("UnderlyingOpenQuote", np.nan), row.get("UnderlyingCloseQuote", np.nan)
...
def parse_file(upload, name):
    ...
    # Accept alternate export naming
...
if "Date" not in df.columns:
    ...
    if "Daily_PL" not in df.columns:
        ...
        if alt:
            ...
            raise ValueError(f"{name}: missing Daily_PL or P/L column.")
...
def infer_probs(daily, vix, vix_change, gap, rv):
    ...
    if vix >= 25: p["Vol Expansion"] += 0.35
    elif vix >= 20: p["Vol Expansion"] += 0.18
    elif vix <= 15:
        p["Compression"] += 0.12
        p["Normal"] += 0.08

    if vix_change >= 1.0: p["Vol Expansion"] += 0.30
    elif vix_change >= 0.5: p["Vol Expansion"] += 0.15
    elif vix_change <= -0.75: p["Compression"] += 0.25
    elif vix_change <= -0.25: p["Compression"] += 0.10
    ...
    if rv >= 20: p["Vol Expansion"] += 0.25
    elif rv >= 15: p["Vol Expansion"] += 0.10
    elif rv <= 10:
        p["Compression"] += 0.15
        p["Normal"] += 0.10
...
with c2: greenday_file = st.file_uploader("Greenday", type=["csv", "xlsx"])
with c3: weak_file = st.file_uploader("Weak", type=["csv", "xlsx"])
with c4: power_file = st.file_uploader("Power Hour", type=["csv", "xlsx"])
...
signals, charts, perf = build_signals(daily, probs, max_capital, target_vol, min_mult, max_mult, exploration_pct, kurt_cut, dd_cut, vol_multiple)
display_cols = [
...
st.header("6. Export")
st.download_button("Download Today’s Signal CSV", signals.to_csv(index=False).encode("utf-8"), file_name=f"v5_exploration_signal_{datetime.now().date()}.csv", mime="text/csv")
st.download_button("Download Regime Probabilities CSV", probs.to_csv(index=False).encode("utf-8"), file_name="regime_probabilities.csv", mime="text/csv")
st.download_button("Download Regime Performance CSV", perf.to_csv(index=False).encode("utf-8"), file_name="strategy_regime_performance.csv", mime="text/csv")
